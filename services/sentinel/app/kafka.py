"""Kafka lifecycle for SENTINEL: pure Kafka-in/Kafka-out (Vector owns the
OpenSearch write path). Same connect/retry/topic-bootstrap pattern as ARGUS.

auto_offset_reset="latest", deliberately unlike ARGUS: ARGUS replays the
queue's ~6h retention on first boot because backdated seed traffic must land
in its baselines. SENTINEL ships a pre-trained model and needs no history —
an "earliest" first boot would re-score hours of backlog (including any
attack soak already handled) and page a burst of stale, old-timestamped
alerts. Committed offsets still resume normally across restarts; this only
governs a brand-new consumer group.
"""
import asyncio
import json
import logging
